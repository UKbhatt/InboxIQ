...
async function getEmails(req, res) {
  try {
    ...
    const updatedAfter = req.query.updated_after || null; // For delta sync

    // Trigger lazy sync in background (non-blocking) for real-time updates
    // Only sync if user is viewing inbox (most important for real-time)
    if (type === 'inbox' || !type) {
      emailService.syncNewEmails(req.user.id).catch(err => {
        console.error('Lazy sync error:', err.message);
      });
    }

    const { emails, total } = await emailService.getEmails(req.user.id, { 
      limit, 
      offset, 
      type,
      updatedAfter 
    ...
    const formattedEmails = emails.map(email => ({
      ...
      updated_at: email.updated_at || email.date, // Include for delta sync
    }));
